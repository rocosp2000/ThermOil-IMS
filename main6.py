...
def initialize_db(db_path):
    try:
        ...
        # Database file not found or unable to connect
        messagebox.showerror("Database Error", "Unable to connect to the database. Please select a different database file.")
        new_db_path = select_database()
        if new_db_path:
            save_config(new_db_path)
            initialize_db(new_db_path)
        else:
            root.quit()  # Exit the program if no database is selected

# Function to select or create a database file
def select_database():
    response = messagebox.askyesno("Database Selection", "Would you like to open an existing database?")
    if response:
        # User chose to open an existing database
        file_path = filedialog.askopenfilename(title="Open Database File", filetypes=[("SQLite Database", "*.db")])
        if not file_path:
            messagebox.showwarning("Warning", "A database file must be selected.")
            return select_database()
        return file_path
    else:
        # User chose to create a new database
        file_path = filedialog.asksaveasfilename(title="Create Database File", filetypes=[("SQLite Database", "*.db")], defaultextension=".db")
        if not file_path:
            messagebox.showwarning("Warning", "A database file must be created.")
            return select_database()
        return file_path

# Function to load the database path from a config file
def load_config():
    try:
...
# Function to save the database path to a config file
...
def setup_gui(root):
    ...
    def edit_item_gui():
        ...
        if item_data:
            ...
            tk.Label(edit_window, text="Item Name", font=("Arial", 14)).pack()
            name_entry = Entry(edit_window, width=30)
            name_entry.insert(0, item_data[0])
            name_entry.pack()

            tk.Label(edit_window, text="Quantity", font=("Arial", 14)).pack()
            ...
            tk.Button(edit_window, text="Save Changes", font=("Arial", 14), command=save_changes).pack()
        else:
    ...
    # Divider before database button
    divider3 = tk.Frame(root, height=2, bg="grey", relief="sunken")
    divider3.pack(fill="x", padx=5, pady=20)

    # Button to change the database
    change_db_button = tk.Button(root, text="Change Database", font=("Arial", 14), command=change_database)
    change_db_button.pack()
...
# Function to change the database file, with a slight modification
def change_database():
    global db_path
    new_db_path = select_database()
    if new_db_path:
        db_path = new_db_path
        save_config(db_path)
        messagebox.showinfo("Database Changed", "The database has been changed. Restart the application to use the new database.")
    else:
        root.quit()  # Exit the program if no new database is selected

# Then load or select the database file
db_path = load_config()
